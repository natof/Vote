...
use natof\vote\librairie\forms\ModalForm;
use natof\vote\librairie\gui\inventories\SimpleChestInventory;
use natof\vote\manager\VoteManager;
use natof\vote\Vote;
...
class VoteCommand extends Command
{
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        ...
        $json = json_decode(file_get_contents("http://minelance-vote.test/api/v1/vote/check/" . $data->get("key") . "/" . strtolower($sender->getName())), true);
        if (isset($args[0]))
        {
            if ($args[0] == "setup")
            {
                if (Server::getInstance()->isOp($sender->getName()) || $sender->hasPermission("vote.use")) {
                    $sender->sendForm(new MenuForm("§9§l« §r§fVote config §9§l»", "", [
                        ...
                        new Button("Key", new Image("textures/items/tripwire_hook"))
                    ...
                    return ;
                } else {$sender->sendMessage($data->get("command_permission_message"));}
            }
        }
        if ($json == null) {$sender->sendMessage("§cThe key is not good contact a server admin!"); return;}
        switch ($json["code"])
        {
            case 0:
